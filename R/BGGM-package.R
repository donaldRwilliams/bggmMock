#' bggmMock:  Bayesian Gaussian Graphical Models
#'
#'
#' @description The \code{R} package \strong{bggmMock} provides tools for making Bayesian inference in
#' Gaussian graphical models (GGM). The methods are organized around two general approaches for
#' Bayesian inference: (1) estimation \insertCite{Williams2019}{bggmMock} and (2) hypothesis testing
#' \insertCite{Williams2019_bf}{bggmMock}. The key distinction is that the former focuses on either the
#' posterior or posterior predictive distribution, whereas the latter focuses on model comparison
#' with the Bayes factor.
#'
#' The methods in \strong{bggmMock} build upon existing algorithms that are well-known in the literature.
#' The central contribution of \strong{bggmMock} is to extend those approaches:
#'
#' \enumerate{
#'
#' \item Bayesian hypothesis testing with the novel matrix-F prior distribution \insertCite{Mulder2018}{bggmMock}
#'
#'  \itemize{
#'
...
#'  }
#'
#' \item Comparing GGMs \insertCite{williams2020comparing}{bggmMock}
#'
#'  \itemize{
#'
...
#' \itemize{
#'
#' \item Predictability with Bayesian variance explained \insertCite{gelman_r2_2019}{bggmMock}
...
#' package \strong{Rcpp} \insertCite{eddelbuettel2011rcpp}{bggmMock} and the \code{c++}
#' library \strong{Armadillo} \insertCite{sanderson2016armadillo}{bggmMock}, there are plotting functions
#' for each method, control variables can be included in the model, and there is support for
#' missing values \code{\link{bggmMock_missing}}.
#'
#' \bold{Supported Data Types}:
#'
#' \itemize{
#'
#' \item Continuous: The continuous method was described \insertCite{@in  @Williams2019_bf;textual}{bggmMock}.
#'
#' \item Binary: The binary method builds directly upon \insertCite{@in @talhouk2012efficient;textual}{bggmMock},
#'       that, in turn, built upon the approaches of \insertCite{lawrence2008bayesian;textual}{bggmMock} and
#'       \insertCite{webb2008bayesian;textual}{bggmMock} (to name a few).
#'
#' \item Ordinal: Ordinal data requires sampling thresholds. There are two approach included in \bold{bggmMock}: (1)
#' the customary approach described in \insertCite{@in @albert1993bayesian;textual}{bggmMock} (the default) and
#' the 'Cowles' algorithm described in \insertCite{@in @cowles1996accelerating;textual}{bggmMock}.
#'
#'
#' \item Mixed: The mixed data (a combination of discrete and continuous) method was introduced
#' \insertCite{@in @hoff2007extending;textual}{bggmMock}. This is a semi-parametric copula model
...
#'
#'  The primary focus of \code{bggmMock} is Gaussian graphical modeling (the inverse covariance matrix).
...
#' to \insertCite{@page 2364, section 2.2, in  @webb2008bayesian;textual}{bggmMock}.
#'
#'
#' \strong{High Dimensional Data?}
#'
#' \strong{bggmMock} was builit specificially for social-behvarioal scientists. Of course, the methods
...
#' data may be added to \strong{bggmMock}.
#'
#' @references
#' \insertAllCited{}
#'
